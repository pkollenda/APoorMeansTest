...
household_head <- read_dta("Data/GSEC2.dta")[c("HHID", "PID", "h2q3", "h2q4", "h2q8", "h2q10")] %>% 
  filter(h2q4 == 1) %>% 
  select(-h2q4) %>% 
  ...
  left_join(read_dta("Data/GSEC4.dta")[c("HHID", "PID", "h4q5", "h4q7", "h4q9")], by = c("HHID", "PID")) %>%
  ...
  left_join(read_dta("Data/GSEC8.dta")[c("HHID", "PID", "h8q4", "h8q5", "h8q6", "h8q7", "h8q8", "h8q9", "h8q10", "h8q11", "h8q12", "h8q13", "h8q14", "h8q15")], by = c("HHID", "PID")) %>%
...
household_composition <- read_dta("Data/GSEC2.dta")[c("HHID", "PID", "h2q3", "h2q8")] %>% 
...
household_educ <- read_dta("Data/GSEC4.dta")[c("HHID", "PID", "h4q5", "h4q7", "h4q9")] %>%
  mutate(highest_educ = case_when(
    PID == "115300270402" ~ 2,
...
household_living <- read_dta("Data/GSEC9A.dta")[c("HHID", "h9q4", "h9q5", "h9q6", "h9q7", "h9q22", "h9q3")] %>% 
...
household_energy <- read_dta("Data/GSEC10B.dta") %>% 
  ...
  full_join(read_dta("Data/GSEC10A.dta") %>% transmute(HHID, electricity = h10q1 == 1, kitchen_room = h10q12 == 1), by = "HHID") %>% 
  # Add information on stove availability
  full_join(read_dta("Data/GSEC10B.dta") %>% group_by(HHID) %>% summarise(stove_any = any(h10q8_1 %in% c(1, 2, 3, 4, 5, 6, 7) & h10q8_2 == 1)) %>% ungroup(), by = "HHID")

household_assets <- read_dta("Data/GSEC14.dta")[c("HHID", "h14q2", "h14q3")] %>% 
...
households <- read_dta("Data/GSEC1.dta")[c("HHID", "region", "urban", "month", "mult")] %>% 
  ...
  full_join(read_dta("Data/UNPS 2011-12 Consumption Aggregate.dta")[c("HHID", "cpexp30", "welfare", "poor")], by = "HHID") %>%
  ...
  mutate(hhsize_fct = cut(hhsize, c(1, 3, 5, 7, 9, Inf), right = FALSE),
         head_age_fct = cut(head_age, seq(14, 105, 5), right = FALSE),
...
saveRDS(households, "Data/prepared/households")
# Save as Stata dta file, open in Stata
write_dta(households, "Data/prepared/households.dta")
