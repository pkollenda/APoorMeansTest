...
1. **Modelling**:
    ...
    \end{align}
    $$
Choice of outcome (consumption or poverty) and model (Verme: OLS/Logit + [Random Forest](#randomforest) and [LASSO](#lasso))<sup>1</sup>: 6 models
...
- For fixed poverty line at $z=F^{-1}(0.2)$ and fixed poverty rate at H=0.2, I calculate Headcount, IER and EER or TER using the basic and extended model like in Brown et al. and add LASSO and Random Forest models.
...
## My results
```{r uganda_results, echo = FALSE}
plot = results %>% 
  filter(z == 0.2) %>% 
  ggplot(aes(x = value, y = measure, colour = model, group = model)) +
  facet_wrap(facets = c("covariates"), ncol = 2, nrow = 2) +
  ...
  labs(y = NULL, x = NULL, color = NULL, title = "Performance of different targeting methods." ) +
  theme_minimal()
 
plot
 
 
  #   guides(color = guide_legend(label.position = "top", title.position = "top", direction = "horizontal", order = 1),
  #          alpha = FALSE) 
...
[Back to presentation](#classification)
...
[Back to presentation](#modelling)   
[To Random Forest](#randomforest)
